...
function App() {
  ...
  return (
    <div className="relative">
      <nav className={"sm:w-64 w-1/4 sm:block  fixed top-0 bottom-0 left-0 sm:py-10 py-3  bg-secondary transition-all ease-linear duration-300 " + (popSidebar ? "block" : "hidden")}>
        <div className="flex text-stone-100 items-center justify-center gap-1 sm:text-2xl text-sm font-bold">
          <BiCube />
      ...
      <nav className="fixed w-screen top-0 -z-10 sm:h-32 sm:pl-64 sm:px-8 px-5  py-2 flex items-center sm:justify-end  justify-between transition-all ease-linear duration-300">
        ...
        <div className="sm:h-[76px] sm:px-6 sm:bg-white rounded-lg drop-shadow-[0_20px_70px_rgba(86,89,146,0.10)] flex items-center justify-center sm:gap-3 ">
          <BsPerson className="sm:bg-slate-100 bg-white p-1 sm:w-12 sm:h-12 w-8 h-8 rounded-[50%] sm:p-2 shadow" />
